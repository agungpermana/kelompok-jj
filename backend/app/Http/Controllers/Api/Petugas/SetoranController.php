...
class SetoranController extends Controller
{
    ...
    public function store(Request $request, $jadwalId)
    {
        ...
        $request->validate(['konfirmasi_pengambilan' => 'required|in:ya,tidak','detail_sampah' => 'required|array|min:1','detail_sampah.*.jenis_sampah_id' => 'required|integer|exists:jenis_sampah,jenis_sampah_id','detail_sampah.*.berat_aktual' => 'required|numeric|min:0.01']);
        ...
        $transaksi = DB::transaction(function () use ($request, $jadwal, $petugas, $warga) {
            $transaksi = TransaksiSetoran::create(['pengajuan_id' => $jadwal->pengajuan_id,'jadwal_id' => $jadwal->jadwal_id,'warga_id' => $warga->warga_id,'petugas_id' => $petugas->petugas_id,'tanggal_setoran' => now(),'konfirmasi_pengambilan' => $request->konfirmasi_pengambilan,'status_validasi' => 'menunggu','total_berat_aktual' => 0,'total_poin' => 0]);
            $totalBerat = 0; $totalPoin = 0;
            foreach ($request->detail_sampah as $item) {
                $jenis = JenisSampah::find($item['jenis_sampah_id']); if (!$jenis) continue;
                $harga = $jenis->hargaSampah()->where('status','aktif')->whereDate('berlaku_mulai','<=',now())->where(function($q){$q->whereNull('berlaku_selesai')->orWhereDate('berlaku_selesai','>=',now());})->orderByDesc('berlaku_mulai')->first();
                if (!$harga) $harga = $jenis->hargaSampah()->where('status','aktif')->orderByDesc('berlaku_mulai')->first();
                ...
                $transaksi->detailSetoran()->create(['jenis_sampah_id' => $jenis->jenis_sampah_id,'berat_aktual' => $berat,'harga_satuan' => $hargaPerSatuan,'nilai_poin_per_satuan' => $nilaiPoinPerSatuan,'poin' => $poin]);
                $totalBerat += $berat; $totalPoin += $poin;
            }
            $transaksi->update(['total_berat_aktual' => $totalBerat,'total_poin' => $totalPoin]);
            $jadwal->update(['status_jadwal' => 'selesai']);
            if ($jadwal->pengajuanPenjemputan) $jadwal->pengajuanPenjemputan->update(['status_pengajuan' => 'selesai']);
            return $transaksi;
        });
