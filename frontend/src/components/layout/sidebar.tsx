...
import { LogOut, ChevronDown, ChevronRight, Leaf, Menu, X, PanelLeftClose, PanelLeftOpen } from 'lucide-react';
  ...
  return (
    <>
      ...
      >
        ...
        {/* Navigation */}
        <nav className={`flex-1 overflow-y-auto py-2 sm:py-3 sidebar-scrollbar transition-all duration-300 ${
          collapsed ? 'px-1.5 space-y-1' : 'px-2 sm:px-3 space-y-1'
        }`}>
            ...
            <div key={sectionIdx} className={section.title ? 'mt-3 sm:mt-4 first:mt-0' : ''}>
              ...
              {section.items.map((item) => {
                    ...
                    <div key={item.label}>
                      <button
                        ...
                        className={`group flex w-full items-center rounded-lg transition-all duration-200 ${
                          collapsed ? 'justify-center px-2 py-2' : 'gap-2 sm:gap-2.5 px-2 sm:px-3'
                        } py-1.5 sm:py-2 text-[12px] sm:text-[13px] font-medium ${
                          active
                          ...
                          {item.children!.map((child) => {
                            ...
                            return (
                              <Link
                                ...
                                href={child.href}
                                onClick={() => setSidebarOpen(false)}
                                className={`flex items-center gap-1.5 sm:gap-2 rounded-md px-2 sm:px-2.5 py-1 sm:py-1.5 text-[11px] sm:text-[12.5px] font-medium transition-all duration-200 ${
                                  childActive
                                    ? 'text-[#16a34a] bg-green-50'
                ...
                return (
                  ...
                  >
                    <item.icon
                      className={`h-4 sm:h-[18px] w-4 sm:w-[18px] flex-shrink-0 ${
                    ...
                    {!collapsed && (
                      <>
                        ...
                        {item.badge && (
                          <span className={`flex h-5 min-w-5 items-center justify-center rounded-full px-1 sm:px-1.5 text-[9px] sm:text-[10px] font-bold flex-shrink-0 ${
        ...
        <div className={`border-t border-gray-100 transition-all duration-300 hidden lg:block ${
          collapsed ? 'px-1.5 py-2' : 'px-2 sm:px-3 py-2 sm:py-3'
        }`}>
          <button
            onClick={toggleCollapsed}
            className={`flex w-full items-center rounded-lg px-2 sm:px-3 py-2 text-[12px] sm:text-[13px] font-medium text-gray-500 hover:bg-gray-50 hover:text-gray-700 transition-colors ${
              collapsed ? 'justify-center' : 'gap-2 sm:gap-2.5'
            }`}
        ...
        <div className={`border-t border-gray-100 transition-all duration-300 ${
          ...
          <button
            ...
            className={`flex w-full items-center rounded-lg px-2 sm:px-3 py-2 sm:py-2.5 text-[12px] sm:text-[13px] font-semibold text-red-500 hover:bg-red-50 transition-colors duration-200 ${
              collapsed ? 'justify-center' : 'gap-2 sm:gap-2.5'
            }`}
          >
