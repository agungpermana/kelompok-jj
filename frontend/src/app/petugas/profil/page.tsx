...
export default function PetugasProfilPage() {
  ...
  const populateForm = (data: CurrentUserProfile) => {
    ...
    setEmail(data.email || '');
    if (data.petugas) {
      setNamaPetugas(data.petugas.nama_petugas || '');
      setJenisKelamin((data.petugas.jenis_kelamin as 'L' | 'P') || '');
      setNoTelepon(data.petugas.no_telepon || '');
      setAlamat(data.petugas.alamat || '');
    }
    setPassword('');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!namaPetugas.trim()) {
      errors.nama_petugas = 'Nama petugas wajib diisi.';
    }
    if (!username.trim()) {
    ...
    if (res.success && res.data) {
      ...
      setToast({
        type: 'success',
        message: res.message || 'Profil petugas berhasil diperbarui.',
      });
  ...
  return (
    <div className="max-w-[1400px] mx-auto pb-16 font-sans">
      <PetugasHeader
        title="Profil Saya"
        subtitle="Kelola data pribadi, kontak operasional, dan keamanan akun petugas."
      />
      ...
      {loading ? (
        <div className="bg-white rounded-2xl border border-gray-200/80 p-16 text-center shadow-sm flex flex-col items-center justify-center min-h-[380px]">
          ...
          <h3 className="text-base font-bold text-gray-900">Memuat Profil Petugas...</h3>
          <p className="text-xs font-medium text-gray-500 mt-1">
            Mengambil data operasional petugas Anda.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-4 space-y-6">
            <div className="bg-white rounded-2xl border border-gray-200/80 p-6 shadow-sm overflow-hidden relative">
              ...
              <div className="absolute top-0 left-0 right-0 h-24 bg-gradient-to-r from-[#15803d] to-[#16a34a]" />
              ...
              <div className="relative pt-8 flex flex-col items-center text-center">
                <div className="relative">
                  <div className="rounded-full border-4 border-white shadow-md">
                    <PetugasAvatar size={96} />
                  </div>
                  <div className="absolute bottom-1 right-1 h-5 w-5 rounded-full bg-emerald-500 border-2 border-white flex items-center justify-center" title="Petugas Aktif">
                    <CheckCircle2 className="h-3 w-3 text-white" strokeWidth={3} />
                  </div>
                </div>

                <h2 className="text-lg font-bold text-gray-900 mt-3.5 leading-tight">
                  {namaPetugas || profile?.username || 'Petugas Bank Sampah'}
                </h2>
            ...
            <div className="bg-gradient-to-br from-emerald-50/70 to-green-50/30 rounded-2xl border border-emerald-200/60 p-5">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h4 className="text-xs font-bold text-gray-900">Informasi Operasional</h4>
                  <p className="text-[12px] text-gray-600 mt-1 leading-relaxed">
                    Data nama dan nomor telepon Anda dapat dilihat oleh warga pada jadwal penjemputan untuk koordinasi pengambilan sampah.
                  </p>
                </div>
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="bg-white rounded-2xl border border-gray-200/80 p-6 shadow-sm">
                <div className="flex items-center gap-2.5 pb-4 border-b border-gray-100 mb-5">
                  ...
                  <div>
                    <h3 className="text-sm font-bold text-gray-900">Data Petugas</h3>
                    <p className="text-[11px] text-gray-400">Informasi identitas dan kontak operasional Anda</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="md:col-span-2">
                    ...
                    <input
                      ...
                      className={`w-full px-3.5 py-2.5 rounded-xl border text-xs text-gray-800 transition-all focus:outline-none focus:ring-2 ${
                        fieldErrors.nama_petugas
                          ? 'border-rose-300 focus:border-rose-500 focus:ring-rose-100'
                          : 'border-gray-200 focus:border-[#16a34a] focus:ring-green-100'
                      }`}
                    />
                    {fieldErrors.nama_petugas && (
                      <p className="text-[11px] text-rose-500 mt-1">{fieldErrors.nama_petugas}</p>
                    )}
                  </div>
                  ...
                  <div className="md:col-span-2">
                    ...
                    <textarea
                      ...
                      placeholder="Alamat lengkap domisili petugas"
