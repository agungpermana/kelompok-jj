...
export default function PenukaranPoinPage() {
  ...
  const [tukarStatus, setTukarStatus] = useState<{
    type: 'success' | 'error';
    message: string;
  } | null>(null);
  ...
  */
  ...
  const loadData = async () => {
    ...
    try {
      const [hadiahData, saldoData, riwayatData] = await Promise.all([
        fetchDaftarHadiah(
          currentPage,
          12,
          search,
        ...
        fetchSaldoPoin(),
        fetchRiwayatPenukaran(riwayatPage),
      ]);

      setHadiah(hadiahData.data);
      setPagination(hadiahData.pagination);

      setSaldo(saldoData.saldo_poin);

      setRiwayat(riwayatData.data);
      setRiwayatPagination(riwayatData.pagination);
    } catch (error) {
      console.error('Gagal memuat data penukaran poin:', error);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  */
  ...
  const handleTukar = async () => {
    ...
    if (saldo < selectedHadiah.poin_dibutuhkan) {
      setTukarStatus({
        type: 'error',
    ...
    try {
      ...
      if (result.success) {
        ...
        setTukarStatus({
          type: 'error',
          message:
            result.error || 'Gagal melakukan penukaran',
        });
      }
    } catch (error) {
      console.error(error);
      ...
      setTukarStatus({
        type: 'error',
        message: 'Terjadi kesalahan saat melakukan penukaran',
      });
    } finally {
  ...
  */
  ...
  const getRewardIconStyle = (name: string) => {
    const value = name.toLowerCase();
  ...
  |--------------------------------------------------------------------------
  |
  | Jika API sudah memiliki field image/gambar/image_url,
  | gunakan image tersebut.
  |
  */

  const getRewardImage = (item: HadiahItem) => {
    const reward = item as HadiahItem & {
      image?: string;
      image_url?: string;
      gambar?: string;
  ...
  */
  ...
  const openConfirmation = (item: HadiahItem) => {
  ...
  */
  ...
  return (
    <div className="min-h-screen bg-gray-50">
          ...
          <main className="min-w-0">
            ...
            <div className="mb-5 flex items-center gap-3 rounded-xl border border-green-100 bg-green-50 px-5 py-4">
              ...
              <p className="text-sm text-gray-600">
                Semakin banyak poin yang Anda kumpulkan,
                semakin banyak manfaat yang bisa didapatkan!
              </p>
            </div>
            ...
            <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
              ...
              <div className="overflow-x-auto border-b border-gray-100 px-5 pt-2">
                <div className="flex min-w-max items-center gap-7">
                  ...
                  <button
                    type="button"
                    className="flex h-12 items-center gap-2 border-b-2 border-green-600 px-1 text-sm font-semibold text-green-600"
                  >
                  ...
                  <button
                    type="button"
                    className="flex h-12 items-center gap-2 border-b-2 border-transparent px-1 text-sm text-gray-600 transition-colors hover:text-green-600"
                  >
                ...
                {isLoading ? (
                  ...
                  <div className="flex min-h-[420px] flex-col items-center justify-center text-center">
                    ...
                    <p className="mt-1 max-w-sm text-sm text-gray-500">
                      Silakan coba kategori atau pencarian
                      lainnya.
                    </p>
                  </div>
                  ...
                  <>
                    ...
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                      ...
                      {hadiah.map((item) => {
                        const RewardIcon =
                          getRewardIcon(item.nama_voucher);
                      ...
                      <div className="mt-8 flex items-center justify-center gap-1.5">
                        ...
                        {Array.from(
                          ...
                          <button
                            ...
                            className={`flex h-9 w-9 items-center justify-center rounded-md text-sm font-medium transition-colors ${page === currentPage
            ...
            <div className="relative overflow-hidden rounded-xl border border-green-100 bg-green-50 p-5">
              ...
              <div className="relative z-10 mt-4 space-y-3">
                ...
                {[
                  ...
                  'Admin akan memberikan poin setelah di validasi',
                ].map((tip) => (
                  <div
                    key={tip}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-[1px]">
          ...
          <div className="w-full max-w-md overflow-hidden rounded-2xl bg-white shadow-2xl">
            ...
            <div className="p-6">
              ...
              {tukarStatus && (
                <div
                  className={`mb-4 flex items-start gap-2 rounded-lg p-3 ${tukarStatus.type === 'success'
                    ? 'bg-green-50 text-green-700'
                    : 'bg-red-50 text-red-700'
                    }`}
                >

                  {tukarStatus.type === 'success' ? (
      ...
      {tukarStatus && !showConfirm && (
        <div className="fixed bottom-5 right-5 z-[60]">

          <div
            className={`flex max-w-sm items-center gap-3 rounded-xl border bg-white px-4 py-3 shadow-xl ${tukarStatus.type === 'success'
              ? 'border-green-200'
              : 'border-red-200'
              }`}
          >
            ...
            <p
              className={`text-sm font-medium ${tukarStatus.type === 'success'
                ? 'text-green-700'
                : 'text-red-700'
                }`}
            >
